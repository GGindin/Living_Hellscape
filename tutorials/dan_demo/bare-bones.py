...
# Imports. For those who don't know, Python operates on numerous modules/libraries that you import into the program.
...
# I typically import pygame as "pg" - more shorthand, easier to wield. You'll be using it a lot.
import pygame as pg
...
# It also technically makes things faster to have the color objects loaded into memory once for other objects to reference.
...
# For example, if you want an object to start from the right side of the screen, you can use SCREEN_W instead of manually typing in the position.
SCREEN_W = 1042
SCREEN_H = 768

# Flags are tags you can give to the set_mode() function below, which include things like FULLSCREEN, RESIZEABLE, and SCALABLE.
# We'll be adding these later-on, but for now we'll put down '0' to tell it we have no flags.
flags = 0
...
# I recommend looking at display.set_mode in the Pygame CE documentation
...
# Player sprite class. Obviously the player object that is currently controlled with the keyboard.
class Player(pg.sprite.Sprite):

    # Initialize function. Polymorphed from the pg.sprite.Sprite() superclass.
    ...
    def __init__(self):
        ...
        # pg.Surface() object is essential to pygame - these are the surfaces that your images will be drawn on.
        # This right now is simply a surface filled with a background color, but later on, these surfaces can be created directly from the image itself.
        ...
        # Starting position. Not necessary, but recopmmended; otherwise your sprite will first appear at position (0,0) (upper-left corner).
    ...
    # Update function. Another function polymorphed from the pg.sprite.Sprite() superclass.
    # We call the update() function for any sprite that we want to animate, based on the instructions its given.
    # In this case, movement is determined by 
    def update(self, pressed_keys):

        if pressed_keys[K_UP] or pressed_keys[K_w]: # Note that pressed_keys conditionals do not use the typical x==y structure, and instead use x[y].
        ...
        # Movement constraints. If the player sprite exceeds the boundaries of the screen, this conditional keeps them from moving off-screen.
        if self.rect.x <= 0:
            self.rect.x = 0
        ...
        # Might be essential for diagonal movement that matches the speed of U/D/L/R movement. Disregard for now.
        """
        vel = pg.Vector2()
...
class Rambler(pg.sprite.Sprite):
    ...
    # Note that this __init__() definition includes the arguments 'startX' and 'startY' - this is how you create arguments for your sprite constructors.
    ...
    def update(self):
        ...
        # Everything else (4-20) does nothing. Pads out the movement so they're not as jarrignly shaky.
...
# Note that it has not yet been added to sprite group, and therefore will not be displayed (yet).
player = Player()
...
while RUNNING == True:
    ...
    # These events can include keypresses, as well as system events such quitting the program.
    for event in pg.event.get():
        ...
        if event.type == KEYDOWN:
            if event.key == K_ESCAPE:
                RUNNING = False # Our first instance of the RUNNING constant being used to end the game.
    ...
    # This is to say that a new frame of video needs to be created using any modifications you made when updating the sprites.
    pg.display.flip()

    # This is where the Clock() object comes into play.
    # The method below constrains video processing to 60 FPS, which is the standard FPS of games made in the NTSC (North America/Japan) format.
    clock.tick(60)

# Quit call. Once everything is finished (i.e. the game loop has ended), the games shuts the window down and closes the program.
pg.quit()
